...
<div class="row g-4">
  <div class="col-md-7">
    <div class="card p-4 shadow-sm">
      ...
      <!-- RESULTS: display model prediction (read-only, styled prominently) -->
      <div class="card mt-4 border-0 bg-light shadow-sm" style="background-color: #f8f9fa;">
        <div class="card-body">
          <h5 class="mb-4 fw-bold text-dark">📊 AI Analysis Result</h5>
          <div class="row g-3">
            <div class="col-md-6">
              <label class="form-label fw-semibold text-dark">Condition</label>
              <div id="resultConditionDisplay" class="form-control fw-bold text-center" style="border: 2px solid #007bff; background-color: #e7f3ff; color: #0056b3; padding: 12px; font-size: 1.1rem;">
                <span id="conditionText">Upload an image to analyze</span>
              </div>
            </div>
            <div class="col-md-6">
              <label class="form-label fw-semibold text-dark">Severity Level</label>
              <div id="resultSeverityDisplay" class="form-control fw-bold text-center" style="border: 2px solid #28a745; background-color: #e8f5e9; color: #1b5e20; padding: 12px; font-size: 1.1rem;">
                <span id="severityText">Upload an image to analyze</span>
              </div>
            </div>
          </div>
          <small class="d-block mt-3 text-muted text-center">Results are automatically detected by the AI model</small>
        </div>
      </div>
...
let currentCondition = 'Onychomycosis'; // default condition
let currentSeverity = 'Mild'; // default severity from AI

function updateResultDisplay() {
  conditionText.textContent = currentCondition;
  severityText.textContent = currentSeverity;
}
...
imageInput.addEventListener('change', () => {
  ...
  simulateProgress(async () => {
    try {
      ...
      // Update condition and severity display from AI prediction
      // Determine condition: if severity is N/A, it's Healthy; otherwise Onychomycosis
      if (data.severity) {
        currentSeverity = data.severity;
        currentCondition = (data.severity === 'N/A') ? 'Healthy' : 'Onychomycosis';
        updateResultDisplay();
      }

      if (data.osi !== undefined && data.osi !== null) {
        // show a quick toast with OSI
        showToast('OSI score: ' + data.osi + ' — ' + (data.severity || ''), 'success', 'fa-chart-line');
      }

    } catch (err) {
      console.error('Prediction failed', err);
      showToast('Prediction failed', 'danger', 'fa-triangle-exclamation');
    }
  });
});
...
saveBtn.addEventListener('click', async () => {
  ...
  const condition = currentCondition;  // AI-determined condition (read-only)
  const severity = currentSeverity;    // AI-determined severity (read-only)
