...
<div class="modal fade" id="editStockModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <label for="editStockQty" class="form-label">Quantity</label>
        <input type="number" id="editStockQty" class="form-control" required min="0">
      </div>
      <div class="modal-footer">
        <button class="btn btn-success" id="saveStockChanges">Save</button>
      </div>
    </div>
  </div>
</div>
  ...
  card.innerHTML = `
    <div class="card shadow-sm border-0 p-3 d-flex flex-row align-items-center justify-content-between">
      <div class="flex-grow-1 me-3">
        ...
        <button class="btn btn-outline-primary btn-sm" onclick="openEditStockModal(${med.id}, '${med.name.replace(/'/g,"\\'")}', ${med.stock})">
          <i class="fa-solid fa-pencil"></i> Edit Stock
        </button>
      </div>
      ${imageArea}
    </div>`;
...
function openEditStockModal(id, name, currentStock) {
  const stockInput = document.getElementById('editStockQty');
  stockInput.value = currentStock || 0;
  stockInput.setAttribute('data-med-id', id);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('saveStockChanges').addEventListener('click', async () => {
    const stockInput = document.getElementById('editStockQty');
    const id = stockInput.getAttribute('data-med-id');
    ...
    try {
      ...
      showToast("Update failed", "danger", "fa-triangle-exclamation");
    }
  });
  ...
  document.addEventListener("click", e => {
    if (e.target.classList.contains("edit-stock")) {
      ...
      document.getElementById("editStockQty").value = item.stock;
      selectedInventoryId = id;
      new bootstrap.Modal(document.getElementById("editStockModal")).show();
    }
  });

  document.getElementById("saveStockChanges").onclick = () => {
    const newStock = document.getElementById("editStockQty").value;
    fetch(`/api/inventory/${selectedInventoryId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
