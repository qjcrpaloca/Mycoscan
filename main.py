...
# YOLO model (lazy import to avoid heavy deps during simple CLI tasks)
try:
    from Models.yolov11_segment import NailSegmentation
except Exception:
    NailSegmentation = None
...
def create_app() -> Flask:
    ...
    # Load model once at startup and attach to app for use by API endpoints.
    app.yolo_model = None
    app.yolo_ready = False
    if NailSegmentation is not None:
        try:
            model_path = os.path.join(app.root_path, "Models", "best.pt")
            print(f"Loading YOLO model from: {model_path}")
            app.yolo_model = NailSegmentation(model_path)
            app.yolo_ready = True
        ...
        print("NailSegmentation class not available; YOLO not initialized.")
    ...
    def require_login():
        ...
        public_routes = [
            'login_page.login', 'login_page.register', 'landing_page.landing',
            # allow API prediction endpoint without login (used by scan page upload)
            'scans_api.predict',
        ]
